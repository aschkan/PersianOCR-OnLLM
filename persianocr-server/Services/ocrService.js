...
const STRUCTURE_PROMPT = [
  ...
  '5) Never invent or pad digits. Copy amounts exactly; use null when unsure. Persian amounts group digits in THREES (۵،۰۰۰،۰۰۰ = 5,000,000) — count the groups carefully, a mis-grouped separator adds or drops zeros.',
  '6) TABLE ROWS ARE ITEMS. When the document has a table of payments/items (ردیف/شماره چک/مبلغ columns), EVERY row goes into items[] with its own مبلغ as "total" — never return an empty items[] for a tabular document. The row amounts must sum to the grand total.',
  '7) invoiceNumber is the document serial (شماره), NEVER a date — dates (۱۳۹۵/۰۸/۱۳) contain / and belong only in "date".',
  '',
